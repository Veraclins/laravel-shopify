<div>
    ...
    <div class="pb-5 flex justify-between items-center">
        <x-jet-input id="email" class="block mt-1 w-full" type="email" name="email" wire:model="search" required
            placeholder="Enter your search here..." autofocus />
        <x-jet-button class="ml-4" wire:click="searchOrders">
            {{ __('Search') }}
        </x-jet-button>
    </div>
    <div wire:loading.remove>
        @if (!$searched)
        <div class="text-2xl">
            Matching orders will show here
        </div>
        @else
        @if (!$orders)
        <div class="text-2xl">
    ...
    <div wire:loading wire:target="searchOrders" class="text-2xl">
        Loading...
    </div>
</div>
